...
$mysql = mysqli_query($connect, "SELECT orders.*,user.*,orders.status FROM `orders` Inner join user on orders.Customer_id = user.id Where orders.Order_id = '$customer'");
...
$tokenid = $row['token'];
$statussss = $row['status'];
$Payment_status = $row['Payment_status'];
 }


$mysqlSSS = mysqli_query($connect, "SELECT `id`, `psgcCode`, `citymunDesc`, `regDesc`, `provCode`, `citymunCode` FROM `city` Where psgcCode = '71218000'");
 while($rowSSSS =$mysqlSSS->fetch_assoc())          {

$citymunDesc = $rowSSSS['citymunDesc'];
$citymunCode = $rowSSSS['citymunCode'];
 }

$mysqlSS = mysqli_query($connect, "SELECT `id`, `brgyCode`, `brgyDesc`, `regCode`, `provCode`, `citymunCode` FROM `brgy` WHERE brgyCode = '$mybrgy'");
 while($rows =$mysqlSS->fetch_assoc())          {

$brgyDesc = $rows['brgyDesc'];

 }
$full_address = $address.' '.$brgyDesc.'. '.$citymunDesc;
$mysql = "SELECT * FROM `delivery_fee` WHERE delivery_fee.City = '$mybrgy'";
      ...
      <div class="container-fluid">
        <div class="row">
          <div class="col-12">
           ...
           style="opacity: .8" width="40"> Mac Ka-On.
...
<?php $mysql = mysqli_query($connect, "SELECT menu.name,menu.photo, orders.*  FROM `orders` inner Join menu on orders.Menu_id = menu.id Where orders.Order_id = '$customer'");
 while($row =$mysql->fetch_assoc())          {
 $Quantity = $row['Menu_QTY'];
            $Name = '<span style="font-size:16px; float:left;">'.$row['name'].'</span>&nbsp; &nbsp;<b><span style="font-size:12px; text-align:center;">'.$row["size"].'</b></span> <b><span style="font-size:16px; float:right;">'.$Quantity.'x</b></span>';
...
$payment_gcash = $total + $myfees;
           ?>
              ...
              <div class="row no-print">
                 ...
                 <?php if($statussss =='deliver'){?>
                 
                
 <form  id="delivered_orders" enctype="multipart/form-data" >
                     <input type="text" name="id" id="id" value="<?php echo  $customer;?>" hidden=""> 
                     
               
                  <button type="submit" class="btn btn-success float-right " ><i class="far fa-credit-card"></i> Order Delivered
                  </button>
                </form>
 
                     <?php }elseif($statussss =='ordered'){ ?>
                     <form id="confirm_order"  enctype="multipart/form-data" >
                     <input type="text" name="id" id="id" value="<?php echo  $customer;?>" hidden=""> 
                    
                  <button type="submit" class="btn btn-success float-right "><i class="far fa-credit-card"></i> For Delivery
                  </button>
                 
                </form>
                <?php }else{ echo'<h4>Order Already Been Completed</h4>'; }?>
                </div>
              </div>
            </div>
            <!-- /.invoice -->
...
$(document).on('submit', '#update_form', function(event){
 ...
 $(document).on('submit', '#delivered_orders', function(event){
  event.preventDefault();
  ...
  if (result.isConfirmed) {
   ...
   $.ajax({
    url:"update/delivered_orders.php",
    method:'POST',
    data:new FormData(this),
    contentType:false,
    processData:false,
