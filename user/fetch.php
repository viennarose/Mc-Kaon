...
try {
    $output = ['data' => []];
    $i = 0;
    $stmt = $connect->query("SELECT menu.name, menu.photo, orders.*  FROM `orders` inner Join menu on orders.Menu_id = menu.id Where orders.Customer_id = '$mydid' AND orders.status ='Cart' ");
    $stmt->execute();
    $t_sn = 1;
